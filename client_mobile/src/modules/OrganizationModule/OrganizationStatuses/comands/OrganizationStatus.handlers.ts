...
@InjectableDI([OrganizationStatusRepository,UseCaseOrganizationStatus,OrganizationStatusModel,OrganizationModel])
	...
	async handlerOrganizationStatus(){
		try {
			if(this.organizationModel.selectOrganization){
				...
				const requestResult = await this.organizationStatusRepository.repositoryOrganizationStatus(point.guid)
				const result:IPointStatus | null = this.useCaseOrganizationStatus.getOrganizationStatus(requestResult)
				const statusMetod =  this.useCaseOrganizationStatus.organizationStatusMetods(result,point)
