		...
		organizationComandBus.handlersComandSubject.subscribe(async (data: any) => {
				try {
					if (data && data.length !== 0) {
						...
						const reduseOrg = await Promise.all(data.map(async (value:IOrganization) => {
							const status = await this.organizationStatusHandlers.handlerOrganizationsListStatus(value);
