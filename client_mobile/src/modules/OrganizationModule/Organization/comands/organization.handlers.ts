...
import { IOrganization, IOrganizationResponse, pointSerch } from 'modules/OrganizationModule/Organization/interfaces/organization.type';

@InjectableDI([UseCaseOrganization,OrganizationRepository,OrganizationStatusHandlers])
export class OrganizationHandlers{
	...
	async  handlerFilterOrganization(filter:string[],cityid:string){
		try {
			const orglist = await this.organizationRepository.repositoryFilterOrganization(filter,cityid)
