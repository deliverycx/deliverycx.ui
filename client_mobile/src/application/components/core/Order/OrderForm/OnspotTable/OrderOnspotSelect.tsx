...
const OrderOnspotSelect: FC<IProps> = ({ onSpotTable, organization, set }) => {
  ...
  const [selectTable, setSelectTable] =
    useState<IOrderOnspotTable>(onSpotTable);
    ...
    <div className="d-flex flex-center gap-8">
      ...
      <div className="input__item input_icon input_icon_right flex-small">
        ...
        >
          ...
          <img
            src={require('assets/images/icons/keyboard_arrow_down_gray_999.png')}
            alt=""
          />
        </div>
        {select && onSpotTable && (
          ...
          <div className="modal modal-order-placement">
            <div className="modal__wrapper">
              ...
              <div className="modal__content gap-8">
                <h3>
                  Ваша корзина наполнена блюдами, которые точно есть в наличии
                  по адресу:
                </h3>
                <h4>{organization.info.address}</h4>
                <small>
                  Если вы хотите изменить заведение вам нужно будет заново
                  заполнить корзину. Продолжить?
                </small>
                <NavLink
                  to={ROUTE_APP.MAIN}
