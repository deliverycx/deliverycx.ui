...
import {
  orderCreateUseCase,
  orderModel,
} from 'modules/OrderModule/order.module';
  ...
  useEffect(() => {
    if (profile && profile.personal) {
      profile.personal.phone &&
        formik.setFieldValue('phone', profile.personal.phone);
      profile.personal.name &&
        formik.setFieldValue('name', profile.personal.name);
    }
  }, [profile]);
    ...
    if (selectDeliveryTipe) {
      switch (selectDeliveryTipe.metod) {
        case DELIVERY_METODS.COURIER:
          setBuilder({
            current: FormBuilderTabsOrder.delivery(OrderFormPayMetods),
          });
          break;
        case DELIVERY_METODS.PICKUP:
          setBuilder({
            current: FormBuilderTabsOrder.pickup(OrderFormPayMetods),
          });
          break;
        case DELIVERY_METODS.ONSPOT:
          setBuilder({
            current: FormBuilderTabsOrder.onspot(OrderFormPayMetods),
          });
          break;
        default:
          setBuilder({ current: null });
      }
    } else {
