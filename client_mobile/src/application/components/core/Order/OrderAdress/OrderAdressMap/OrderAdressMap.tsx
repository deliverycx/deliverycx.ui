...
import {
  ...
  Mobile,
  isDesctomMediaQuery,
} from 'application/ResponseMedia';
  ...
  useEffect(() => {
    if (orderDeliveryAddress) {
      formik.setFieldValue(
        'address',
        `${orderDeliveryAddress.city}, ${orderDeliveryAddress.address} ${orderDeliveryAddress.house}`,
      );
    }
  ...
  const handlerMap = () => {
    if (user && user.phone) {
      ...
      if (orderDeliveryAddress) {
        navigate(
          ROUTE +
            `/${orderDeliveryAddress.address}${orderDeliveryAddress.house}?query=${JSON.stringify(orderDeliveryAddress)}`,
        );
      } else {
        navigate(ROUTE);
      }
    ...
    <>
          ...
          <div className="input__container">
            ...
            {orderDeliveryAddress ? (
              <input
                readOnly
                onClick={handlerMap}
                placeholder="Нажмите для выбора адреса"
                value={`${orderDeliveryAddress.city},ул ${orderDeliveryAddress.address}`}
                name="address"
                type="text"
              />
            ) : (
      ...
      {isModalOpened && (
        <>
                  ...
                  <div className="delivery-addresses__list__items">
                    {profile && profile.adressdelivery.length === 0 && (
                      <div className="delivery-addresses__list__items-empty">
                        <h3>Нет адресов</h3>
                        <span>
                          Добавьте адрес доставки для быстрого оформления ваших
                          заказов
                        </span>
                      </div>
                    )}
                    {profile &&
                      profile.adressdelivery &&
                      profile.adressdelivery.map((val) => {
                        const CN = cn('delivery-addresses__list__items-item', {
                          active: val.address === orderDeliveryAddress?.address,
                        });
                        ...
                        return (
                          city?.cityname === val.city && (
                            <div key={val.address} className={CN}>
                              <div
                                className="delivery-addresses__list-box"
                                onClick={() => handlerSelect(val)}
                              >
                                <div className="order-placement__payment-method__item-btn">
                                  {val.address ===
                                    orderDeliveryAddress?.address && (
                                    <img
                                      src={require('assets/images/icons/doneok.png')}
                                    />
                                  )}
                                </div>
                                ...
                                <span>
                                  {' '}
                                  ул. {val.address}, {val.house}
                                </span>
                              </div>

                              <button
                                className="btn btn-none btn-mini"
                                onClick={() => userEditAddes(val)}
                              >
                                <svg
          ...
          <Desktop>
            <ModalCard
              setIsOpened={() => setIsModalOpened(false)}
              theme={null}
              styles="order_adressmodal-desc"
            >
              <div className="modal__wrapper map__institute-info">
                <div className="delivery-addresses__list">
                  <div className="delivery-addresses__list__items">
