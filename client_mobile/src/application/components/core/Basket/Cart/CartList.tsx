      ...
      <div className="basket__content__product_card">
        <label className="input__checkbox">
          <div className="checkbox">
            <input
              type="checkbox"
              checked={select.includes(product.id)}
              onChange={(e) => choise(e, product.id)}
            />
            <svg
              width="14"
              height="11"
        ...
        <div className="basket__content__product_card__info">
          ...
          <div className="basket__content__product_card__info__buttons">
            <button
              onClick={() => basketUseCase.removeOneCart(product.id)}
              className="btn btn-mini btn-gray"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
          ...
          {error && <div className="cart__item__validate">{error}</div>}
        </div>
      </div>
    </>
  );
