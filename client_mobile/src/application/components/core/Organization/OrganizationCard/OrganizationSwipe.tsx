...
const OrganizationSwipe:FC<{organizations: IOrganization[],point:IOrganization}> = ({organizations,point}) =>{
	
	const [sw,setSw] = useState<any>()

	useEffect(()=>{
		if(point){
			const findindex = organizations.findIndex(val => val.guid === point.guid)
