...
const DeliveryAdressSelect: FC<IProps> = ({ setModalStreet }) => {
	...
	const [serchInp, setSerchInp] = useState<string>((stateReduceMap.valueMap && stateReduceMap.valueMap.name) || '')
	...
	useEffect(() => {
		if (!isLoading) {
			(ikkostreet && serchInp) && searchHandle(serchInp, ikkostreet.data as IIkkoStreet[])
		}
	...
	return (
		<div className="delivery-addresses authorized">
			<div className="top-bar">
				...
				<div onClick={() => setModalStreet(false)} className="d-flex flex-center justify-center modal__btn no-drag flex-column">
