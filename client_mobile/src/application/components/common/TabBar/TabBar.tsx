...
const TabBar = () => {
  ...
  const CNCART = cn('tab-bar__link value', {
    tabcart_disable:
      basketModel.basketPrice && basketModel.basketPrice.fullPrice === 0,
  });
  ...
  return (
    <>
      <nav className="tab-bar link-to-cart">
        <ul className="tab-bar__list">
          <li className="tab-bar__item">
            <NavLink
              onClick={scroll}
              to={ROUTE_APP.SHOP.SHOP_MAIN}
              className="tab-bar__link"
            >
              <div className="tab-bar__img">
                <svg
                  className="tab-bar__img-home"
          ...
          {/*
						...
						{user && user.phone ? (
							...
							<NavLink to={ROUTE_APP.AUTH.AUTORIZATE} className="tab-bar__link tabcart_disable">
								<div className="tab-bar__img">
      ...
      {isMenuOpen && <MainMenu closeMenu={setIsMenuOpen} like={like} />}
    </>
  );
};
