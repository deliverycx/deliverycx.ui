...
const CartAdresSelect: FC<IProps> = ({ formik }) => {
	...
	return (
		<div className="onspot_box__select form__field__type payment_section adress_fild__address onspot_box__select-full">
			<div className="onspot_box_dual">
				<div className="onspot_select_def def_street">
			...
			{
				serchInp && streets &&
				<div className="payment_list open">
					{
						streets && streets.map((val: IIkkoStreet) => {
							return <div key={val.id} className="payment-item_box" onClick={() => handlerСhooseAdress(val)} >{val.name}</div>
						})
					}
