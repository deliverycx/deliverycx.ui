...
const CartYmap = () => {
  ...
  return (
    <div className="address-select-map">
      <YMaps
        enterprise
        query={{ apikey: "d52d6907-6e6e-4850-a49f-f77df081ee23" }}
      >
                      ...
                      }
                        {
                            stateReduceMap.disclaimer && <div className="disclaimer">Не точный адрес</div>
									...
									}
									{
										isLoadingStreet ?
										<button
                      disabled={isLoadingStreet}
                      className="mapsPopup__button btn">
                    Загрузка
                    
                  	</button>
										: <button
                      disabled={stateReduceMap.cord && !stateReduceMap.cord.length && !stateReduceMap.disclaimer || stateReduceMap.disclaimer && isLoadingStreet}
